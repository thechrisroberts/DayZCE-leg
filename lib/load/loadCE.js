...
module.exports = {
    ...
    async loadTypesFiles() {
        sets.files.where('type', 'types').first().then(file => {
            console.log(file);
        });

        // await sets.files.where('type', 'types').fetch().then(async files => {
        ...
        // await sets.files.where('type', 'types').fetch().then(async files => {
        //     await files.forEach(async file => await this.loadTypesFile(file));
        // });
    },

...
module.exports = {
    ...
    async loadUsage(usages) {
        for (let i = 0; i < usages.length; i++) {
            await new Usage({ name: usages[i].name }).save().then(usage => sets.meta.usages.add(usage));
        }
    },
    async loadTags(tags) {
        for (let i = 0; i < tags.length; i++) {
            await new Tag({ name: tags[i].name }).save().then(tag => sets.meta.tags.add(tag));
        }
    },
    async loadCategories(categories) {
        for (let i = 0; i < categories.length; i++) {
            await new Category({ name: categories[i].name }).save().then(category => sets.meta.categories.add(category));
        }
    },
    async loadValues(values) {
        for (let i = 0; i < values.length; i++) {
            await new Value({ name: values[i].name }).save().then(value => sets.meta.values.add(value));
        }
    },
};